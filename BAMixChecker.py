...
def make_bed( total_SNP_bed, targeted_bed , OUTDIR, bedtools_path ):
	...
	(exitstatus, line_count) = subprocess.getstatusoutput("cat {0}targetSNPs.bed | wc -l".format(OUTDIR))
...
#		print stderr
	...
	if args.NonHumanSNPlist != "":
		...
		(exitstatus, line_count) = subprocess.getstatusoutput("ls {0}".format(args.NonHumanSNPlist))
		if exitstatus != 0:
			print("# ERROR: Fail to read " + args.NonHumanSNPlist)
			sys.exit(1)
		if args.BEDfile != '':
			(exitstatus, line_count) = subprocess.getstatusoutput("{0} intersect -b {1} -a {2} | wc -l".format(bedtools_path, args.BEDfile, args.NonHumanSNPlist))
		else:
			(exitstatus, line_count) = subprocess.getstatusoutput("wc -l {0}".format(args.NonHumanSNPlist))
