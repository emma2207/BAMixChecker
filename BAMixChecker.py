...
try:
    ...
    from multiprocessing import Process, Array, Pool, active_children
except ImportError:
    sys.exit(
        "## ERROR: Required python package or packages are not installed.\n"
    ...
    if user_file_list != "":
        ...
        for file_path in lis_files_whole:
            ...
            if tmp_file_type not in ["BAM", "CRAM", "SAM"]:
                print(
                    "## ERROR: Input files in the list are needed to be .bam or .cram file"
                )
...
def make_bed(total_SNP_bed, targeted_bed, OUTDIR, bedtools_path):
    os.system(
        "{0} intersect -b {1} -a {2} | uniq > {3}targetSNPs.bed".format(
            bedtools_path, targeted_bed, total_SNP_bed, OUTDIR
        )
    )
    (exitstatus, line_count) = subprocess.getstatusoutput(
        "cat {0}targetSNPs.bed | wc -l".format(OUTDIR)
    )
    if int(line_count) < 200:
        if total_SNP_bed.split("/")[-1] not in [
            "gnomad_hg38_AF10.bed",
            "gnomad_hg19_AF10.bed",
        ]:
            os.system("rm {0}targetSNPs.bed".format(OUTDIR))
            return None
        else:
            print(
                "## WARNING: The target size is too small, so the number of SNP sites to compare is under 200."
            )
            print(
                "##          The specificity could be lower with the small numbr of SNP loci."
            )
...
def run_p(cmm):
    ...
    try:
        ...
        # decode stderr/stdoutput if bytes
...
def run_HC(
    lis_bam_files, OutputDIR, reference_file, bed_file, max_prc, HC_path, ploidy
):
    print("Calling the variants information with GATK HaplotypeCaller")
    print("Max process :", max_prc)
    max_prc = int(max_prc)
    ...
    for i in range(0, len(lis_bam_files)):
        vcf_file = "{0}{1}gvcf".format(
            vcf_file_path, lis_bam_files[i].split("/")[-1][:-3]
        )
        cmm = "{0} HaplotypeCaller -I {1} -O {2} -R {3} -L {4} -ERC BP_RESOLUTION -ploidy {5}".format(
            HC_path, lis_bam_files[i], vcf_file, reference_file, bed_file, ploidy
        )
        lis_cmm.append(cmm)
    ...
    try:
        ...
        print(
            "          Preperation for calling with GATK is instructed in gitHub 'https://github.com/heinc1010/BAMixChecker'."
        )
    ...
    for i in range(0, len(lis_vcf)):
        ...
        for j in range(0, len(lis_vcf)):
            if j < i:
                ...
                procs.append(
                    Process(
                        target=cal_cor_each, args=(lis_vcf[i], lis_vcf[j], cor_arr, j)
                    )
                )
...
def pairing(cor_matrix, lis_files):
    ...
    smp_pairs = {}
    for i in range(0, len(lis_files)):
        smp_pairs[lis_files[i]] = [
            lis_files[j] for j in range(0, len(lis_files)) if cor_matrix[i][j] > 0.7
        ]
    return smp_pairs
...
def get_sw_pairs(lis_files, smp_pairs):
    ...
    for s1 in dic_split_samples.keys():
        ...
        for s2 in dic_split_samples.keys():
            if s1 != s2:
                score = compareVectors(
                    dic_split_samples[s1], dic_split_samples[s2], lis_entropies
                )
        ...
        try:
            ...
            for f in smp_pairs.keys():
                if f1 in smp_pairs[f]:
                    if f not in lis_m_f:
                        dic_sw[f1] = [f]
    lis_sw_keys = list(dic_sw.keys())
    lis_sw_keys.sort()
    for f1 in lis_sw_keys:
        dic_sw[f1].sort()
        for f2 in dic_sw[f1]:
            try:
                if (f1 != f2) and (f1 in dic_sw[f2]):
                    dic_sw[f2].remove(f1)
            except Exception:
                continue
...
def make_result_file_no_file_name_info(
    cor_matrix, smp_pairs, lis_files, OutputDIR, lis_paired_files
):
        ...
        for f2 in smp_pairs[f1]:
            ...
            if lis_tmp not in lis_done:
                ...
                if flag_less_informative:
                    fw_m_m.write(
                        "-> *This pair scores under 0.8 which is less informative. The 'less informative score' dosen't mean that the pair is not matched but may have some problem of purity or copy number variation etc. in the sample.\n"
                    )
    fw_m_m.close()
    ...
    if (lis_ans == []) and (len(lis_files) < 6):
        ...
        if lis_ans == []:
            ...
            if (dic_sw == None) and (dic_un_p == None):
                print(
                    "## WARNING : The file names don't have detectable common regulation."
                )
                print("             Pairing samples only by genotype.")
                make_result_file_no_file_name_info(
                    cor_matrix, smp_pairs, lis_files, OutputDIR, lis_paired_files
        ...
        for i in range(0, len(lis_paired_files)):
            ...
            if len(smp_pairs[f1]) > 1:
                pass
            for f2 in smp_pairs[f1]:
                if (f2 not in dic_sw[f1]) and (f1 not in dic_sw[f2]):
                    lis_tmp = [f1, f2]
                    lis_tmp.sort()
                    if lis_tmp not in lis_done:
                        flag_less_informative = False
...
def mk_html_dic(OutputDIR, lis_m, lis_sw, lis_up):
    ...
    if lis_m != []:
        ...
        m_ums = ",".join(lis_m_um)
        fw_r.write(
            "df.m <-data.frame('Sample1'=c({0}), 'Sample2'=c({1}),'Concordance rate'=c({2}), 'Conclusion'=c({3}))\n".format(
                f1s, f2s, scores, m_ums
            )
        )
        fw_r.write(
            "colnames(df.m) <- c('Sample1', 'Sample2','Concordance rate', 'Conclusion')\n"
        )
    # swapped
    if lis_sw != []:
        lis_f1 = []
        ...
        m_ums = ",".join(lis_m_um)
        fw_r.write(
            "df.sw <-data.frame('Sample1'=c({0}), 'Sample2'=c({1}),'Concordance rate'=c({2}), 'Conclusion'=c({3}))\n".format(
                f1s, f2s, scores, m_ums
            )
        )
        fw_r.write(
            "colnames(df.sw) <- c('Sample1', 'Sample2','Concordance rate', 'Conclusion')\n"
        )
    # unpaired
    if lis_up != []:
        lis_f1 = []
        ...
        m_ums = ",".join(lis_m_um)
        fw_r.write(
            "df.up <-data.frame('Orphan sample'=c({0}), 'Best match by file name'=c({1}),'Concordance rate'=c({2}), 'Conclusion'=c({3}))\n".format(
                f1s, f2s, scores, m_ums
            )
        )
        fw_r.write(
            "colnames(df.up) <- c('Orphan sample', 'Best match by file name','Concordance rate', 'Conclusion')\n"
        )
    fw_r.write("```\n## Mismatched samples\n")
    ...
    if lis_m != []:
        fw_r.write("```{r , results='asis', echo=FALSE}\n")
        fw_r.write(
            "z = ztable(df.m,align='cccc',include.rownames=FALSE)\nprint (z, type = 'html')\n```\n"
        )
    else:
    ...
    fw_r.write(
        "colnames(df.total) <- c('Sample1', 'Sample2','Concordance rate', 'Conclusion')\n"
    )
    fw_r.write("z = ztable(df.total,align='cccc',include.rownames=FALSE)\n")
    if lis_sw != []:
        if lis_up != []:
            fw_r.write(
                "z <-addRowColor(z,c({0}),'pink')\nz <-addRowColor(z,c({1}),'light green')\n".format(
                    ",".join(lis_sw_c), ",".join(lis_up_c)
                )
            )
        else:
        ...
        if lis_up != []:
            fw_r.write(
                "z <-addRowColor(z,c({0}),'light green')\n".format(",".join(lis_up_c))
            )
    fw_r.write("print (z, type = 'html')\n```\n")
    fw_r.close()
    cmm = "Rscript {0} {1} {2}".format(
        os.path.dirname(os.path.realpath(__file__)) + "/r_script.r",
        OutputDIR + "BAMixChecker_Report.Rmd",
        OutputDIR,
    )
    prc = Popen(cmm, stdout=PIPE, shell=True, stderr=PIPE)
    stdoutput, stderr = prc.communicate()
...
def mk_html_no_mismatched(OutputDIR, lis_m):
    ...
    fw_r.write("```{r , results='asis', echo=FALSE}\n")
    fw_r.write(
        "z = ztable(df.total,align='cccc',include.rownames=FALSE)\nprint (z, type = 'html')\n```\n"
    )
    fw_r.close()
    cmm = "Rscript {0} {1} {2}".format(
        os.path.dirname(os.path.realpath(__file__)) + "/r_script.r",
...
def mk_heat_map(OutputDIR, lis_files, cor_matrix):
    ...
    if len(lis_files) < 10:
        ...
        cex = "0.1"
    fw_r.write(
        "corrplot(as.matrix(df.total),method='shade', shade.col=NA, tl.col='black', tl.srt=90,cl.lim = c(0, 1), cl.ratio=0.08, tl.pos='lt',tl.cex ={0})\ndev.off()\n".format(
            cex
        )
    )
    fw_r.close()
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        prog="BAMixChecker",
        description="Sample mix-up checker to detect sample mismatch with pairs of BAM file in a cohort for human WGS/WES/RNA-seq and targeted sequencing.",
    )
    parser.add_argument(
        "-d", "--DIR", default="", help="Directory path of the .BAM or .CRAM files"
    )
    parser.add_argument(
        "-l", "--List", default="", help="A file with the list of .BAM or .CRAM files"
    )
    ...
    parser.add_argument(
        "-b", "--BEDfile", default="", help=".bed file for Targeted sequencing data."
    )
    parser.add_argument(
        "-v",
        "--RefVer",
        default="hg38",
        choices=["hg38", "hg19"],
        help="Version of reference : 'hg19' or 'hg38'. Default = 'hg38'",
    )
    parser.add_argument(
        "-p", "--MaxProcess", default="1", help="The number of max process. Default = 1"
    )
    parser.add_argument(
        "-nhSNP",
        "--NonHumanSNPlist",
        default="",
        help="The SNP list for non-human organism sample matching check-up in BED format.",
    )
    parser.add_argument(
        "-pld",
        "--Ploidy",
        default="2",
        help="The ploidy of sample. Default = 2 for human",
    )
    parser.add_argument(
        "--FullPATH",
        action="store_true",
        help="Use to report with the full path of file. BAMixChecker resports with the only file name as a default.",
    )
    parser.add_argument(
        "--RemoveVCF",
        action="store_true",
        help="Use to remove called germline VCF after running.",
    )
    parser.add_argument(
        "--OFFFileNameMatching",
        action="store_true",
        help="Use to get a result without file-name based pairing.BAMixChecker will only compare smaples by genotype. If the input list of file contains two or more file on a line (means samples from same individual), this option is automatically applied and it use the user-given pair information.",
    )
    ...
    HC_path = ""
    fr_config = open(
        os.path.dirname(os.path.realpath(__file__)) + "/BAMixChecker.config", "r"
    )
    ...
    if args.DIR == "":
        if args.List == "":
            print(
                "## ERROR: There is no information about input files. Use -d or -l option for the input file information."
            )
            sys.exit(1)
    else:
        if args.List != "":
    ...
    cmm = "echo {0} > {1}".format(
        install_check, out_path + "R_packages_install_check.R"
    )
    prc = Popen(cmm, stdout=PIPE, shell=True, stderr=PIPE)
    stdoutput, stderr = prc.communicate()
    ...
    if args.Ref == "":
        ...
        sys.exit(1)
    flag_chr = False
    (exitstatus, header) = subprocess.getstatusoutput("head -1 {0}".format(args.Ref))
    ...
    if args.RefVer in ["hg38", "hg19"]:
        if args.BEDfile != "":
            ...
            if args.NonHumanSNPlist == "":
                bed_file = make_bed(
                    "{0}gnomad_{1}_AF{2}_AF{3}_All.bed".format(
                        bed_file_path, args.RefVer, 45, 35
                    ),
                    args.BEDfile,
                    out_path,
                    bedtools_path,
                ...
                for AF in range(45, 5, -5):
                    for AF_all in range(AF, -1, -10):
                        ...
                        if AF_all != 0:
                            bed_file = make_bed(
                                "{0}gnomad_{1}_AF{2}_AF{3}_All.bed".format(
                                    bed_file_path, args.RefVer, AF, AF_all
                                ),
                                args.BEDfile,
                                out_path,
                                bedtools_path,
                            ...
                            bed_file = make_bed(
                                "{0}gnomad_{1}_AF{2}.bed".format(
                                    bed_file_path, args.RefVer, AF
                                ),
                                args.BEDfile,
                                out_path,
                                bedtools_path,
            ...
            bed_file = "{0}gnomad_{1}_AF45_AF35_All.bed".format(
                bed_file_path, args.RefVer
            )
    else:
    ...
    if args.NonHumanSNPlist != "":
        ...
        (exitstatus, line_count) = subprocess.getstatusoutput(
            "ls {0}".format(args.NonHumanSNPlist)
        )
        if exitstatus != 0:
        ...
        if args.BEDfile != "":
            (exitstatus, line_count) = subprocess.getstatusoutput(
                "{0} intersect -b {1} -a {2} | wc -l".format(
                    bedtools_path, args.BEDfile, args.NonHumanSNPlist
                )
            )
        else:
            (exitstatus, line_count) = subprocess.getstatusoutput(
                "wc -l {0}".format(args.NonHumanSNPlist)
            )
        print(str(line_count) + " SNP loci will be compared.")
        bed_file = args.NonHumanSNPlist

    if args.FullPATH:
        lis_bam_files, lis_ans, lis_bam_full_path = get_file_list(
            dir_path, args.List, True, flag_FNM
        )
    else:
        lis_bam_files, lis_ans, lis_bam_full_path = get_file_list(
            dir_path, args.List, False, flag_FNM
        )
    ...
    result = make_result_file(
        cor_matrix, smp_pairs, lis_bam_files, out_path, lis_ans, flag_FNM
    )
    ...
    if result == 1:
        ...
        print(
            "Swapped file exist. Check 'BAMixChecker_Report.html' or 'Mismatched_samples.txt' file."
        )
